# W5 — TanStack Query 전면 전환 설계 (Frontend UX)

> 작성일: 2026-06-11 · 트랙: 프론트 UX · 선행: W4 (분석 고도화, 로컬 main 머지 / origin 미푸시)
...
> **라이브러리 버전 주의**: Context7 문서 조회가 월간 쿼터 초과로 실패함. 본 설계는 TanStack Query **v5의 알려진 표준 패턴**(object 형식 `useQuery`/`useMutation`, `onMutate`/`onError`/`onSettled` 낙관적 갱신, `QueryClientProvider`)을 기준으로 한다. 구현 단계에서 `pnpm add @tanstack/react-query`로 최신 v5에 핀하고, v5 API와 어긋나는 부분이 있으면 구현자가 공식 문서로 확인해 조정한다.
...
| 훅 | 키 | queryFn | enabled |
|---|---|---|---|
| `useTransactions(filters)` | `qk.transactions(filters)` | `fetchTransactions` | — |
| `useMonths()` | `qk.months()` | `fetchMonths` | — |
| `useSummary(month)` | `qk.dashboard.summary(m)` | `fetchSummary` | `!!month` |
| `useByCategory(month)` | `qk.dashboard.byCategory(m)` | `fetchByCategory` | `!!month` |
| `useCashflow(lastN)` | `qk.dashboard.cashflow(n)` | `fetchCashflowByMonth` | — |
| `useTopMerchants(month, limit)` | `qk.dashboard.topMerchants(m,n)` | `fetchTopMerchants` | `!!month` |
| `useByEssential(month)` | `qk.dashboard.byEssential(m)` | `fetchByEssential` | `!!month` |
| `useInsight(month)` | `qk.insight(m)` | `fetchInsight` | `!!month` |

**Mutation 훅**:

- **`useUploadStatement()`** — `mutationFn`: 기존 `api.post("/transactions/upload", fd)`.
  `onSuccess`: `invalidateQueries`로 `["transactions"]`, `["months"]`, `["dashboard"]` prefix 무효화(부분 키 매칭). 업로드는 어느 월/카테고리에 들어갈지 모르므로 낙관적 갱신 없이 무효화만.

- **`useCategoryOverride()`** — `mutationFn`: `PATCH /transactions/{id}` (기존 api 함수).
  낙관적 갱신:
  - `onMutate({ id, category })`: `cancelQueries(["transactions"])` → 현재 transactions 캐시 스냅샷 저장 → 모든 매칭 캐시에서 해당 row의 `user_category_override`·`effective_category`를 낙관적으로 갱신 → context로 스냅샷 반환
  - `onError(_e, _v, ctx)`: 스냅샷으로 롤백
  - `onSettled`: `invalidateQueries(["transactions"])` + `["dashboard"]`(카테고리 집계 영향)

- **`useEssentialOverride()`** — `mutationFn`: `PATCH /transactions/{id}/essential`.
  낙관적 갱신 동일 패턴: row의 `essential_override` 갱신. `effective_essential`은 서버 파생값이라 낙관적 단계에서는 `essential_override`만 반영하고(W4 배포노트의 알려진 한계와 동일), `onSettled` 무효화로 정합. `onSettled`: `["transactions"]` + `["dashboard"]`(by-essential 영향).

- **`useGenerateInsight()`** — `mutationFn`: `POST /insights/generate?force=true` (body `{month}`).
  `onSuccess(data, { month })`: `setQueryData(qk.insight(month), data)`로 즉시 갱신(재요청 불필요). 503/502 에러는 호출 컴포넌트에서 표시.
...
- 필터 객체: `{ month, category: categories, search: debouncedSearch, limit: 200 }` → `useTransactions(filters)`.
  - 월/카테고리 변경은 즉시 쿼리 키 변경(재요청), 검색만 디바운스.
...
- 카테고리/essential 변경: `useCategoryOverride()`·`useEssentialOverride()`의 `mutate` 호출. 기존 수동 `onCategoryChange`/`onEssentialChange` + `setTxns` 제거. `TransactionList`/`CategoryChip`/`EssentialToggle`의 콜백 prop 시그니처는 유지하고, 내부에서 mutation을 호출하도록 연결(컴포넌트 변경 최소화).
- 로딩 표시는 `useTransactions().isPending`.
...
- `months`/`month` 선택 상태: `useMonths()` 결과로 첫 월 자동 선택. `month`는 로컬 useState로 유지하되 초기값은 `useMonths` 데이터에서 파생(첫 로드 시 `months[0]`).
- 5개 집계: `Promise.all` 제거 → `useSummary(month)`·`useByCategory(month)`·`useCashflow(6)`·`useTopMerchants(month,5)`·`useByEssential(month)` 개별 호출(자동 병렬, 각자 캐시).
- 로딩: 집계 쿼리들의 `isPending` 조합. 에러: `isError` 조합으로 기존 통합 에러 배너 유지.
- 빈 상태("아직 거래가 없습니다"): `useMonths().data?.length === 0`로 판정.
...
쓰기(카테고리/essential 토글)
  → useMutation.mutate
  → onMutate: 캐시 낙관적 패치(즉시 UI 반영)
  → 성공: onSettled invalidate → 서버 정합
  → 실패: onError 롤백
```
...
- **기존 26개 web 테스트**: 대부분 props 기반 컴포넌트 테스트라 유지. `QueryClientProvider`가 필요한 라우트/훅 테스트를 위해 `apps/web/src/test/` 에 `renderWithClient(ui)` 헬퍼 추가(각 테스트마다 `retry: false`인 새 QueryClient).
- **신규 훅 테스트** (`lib/queries.test.tsx`):
  ...
  - `useUploadStatement` onSuccess 시 transactions/months/dashboard 무효화 호출 확인
- **`useDebouncedValue` 테스트** (`lib/useDebouncedValue.test.tsx`): fake timers로 delay 후에만 값 반영 확인
- **라우트 통합**: `/app` 검색 입력 → 디바운스 후 1회만 요청(요청 카운트 assert), 월/카테고리 변경은 즉시 요청
...
- TanStack Query v5 API를 Context7로 확정 못 함(쿼터 초과). 구현 단계에서 공식 문서 대조 필요.
